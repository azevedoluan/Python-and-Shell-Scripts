CREATE OR REPLACE PROCEDURE CARGA_MATERIAL_STAGING IS
	...
	grp_merc_aux		staging_material.grp_merc%TYPE;
	...
	--DADOS ITEM MESTRE
	CURSOR WCUR0 IS
		SELECT	TRIM(A.COD_ASTREIN)									COD_ASTREIN,
				TO_NUMBER(TRIM(A.COD_ANTIGO))						CODIGO_ANTIGO,
				TRIM(A.DESC_CURTA)									DESC_CURTA,
				TRIM(A.NCM)											NCM,
				TO_CHAR(A.DESC_LONGA) 								DESC_LONGA,
				TRIM(A.GRP_MERC)		 							GRP_MERC,
				A.CONTROLE 											CONTROLE,
				CAST(NVL(TRIM(B.IBMCU),' ') AS VARCHAR2( 12 CHAR))	FILIAL,
				NVL(TRIM(B.IBSRP0),' ')								CLASS_MAT,
				NVL(TRIM(A.BLQ), ' ')								BLOQUEIO
		FROM	STAGING_ITEMMESTRE A
		LEFT JOIN	F4102_TONE	B	ON TO_NUMBER(TRIM(A.COD_ANTIGO))=B.IBITM
		LEFT JOIN	F4101_TONE	C ON TO_NUMBER(TRIM(A.COD_ANTIGO))=C.IMITM
		WHERE (TO_DATE(A.DATA_ATUALIZA) >= (SELECT TO_DATE(data_proc-1) FROM STAGING_CONTROLE WHERE DADO = 'MATERIAL') OR TO_DATE(SUBSTR(C.IMUPMJ,2,5),'RRDDD') >= (SELECT TO_DATE(data_proc-1) FROM STAGING_CONTROLE WHERE DADO = 'MATERIAL') OR TO_DATE(SUBSTR(B.IBUPMJ,2,5),'RRDDD') >= (SELECT TO_DATE(data_proc-1) FROM STAGING_CONTROLE WHERE DADO = 'MATERIAL'));
	REC0 WCUR0 %ROWTYPE;
...
BEGIN
	...
	LOOP
		...
		IF CONTADOR > 0 THEN
			...
			UPDATE	STAGING_ITEMMESTRE
			SET		CONTROLE = 'S'
			WHERE	TO_NUMBER(TRIM(COD_ANTIGO)) = REC0.CODIGO_ANTIGO
			AND		COD_ASTREIN = REC0.COD_ASTREIN;
		...
		ELSE
			...
			IF REC0.FILIAL = ' ' THEN
				...
				DTA_ULT_MOV := '01-JAN-1900';
				DTA_MOV := TO_CHAR(DTA_ULT_MOV,'DD/MM/YYYY');
			ELSE
				FILIAL_ERP := REC0.FILIAL;
				
				BEGIN
					SELECT	NVL(MAX(TO_DATE(SUBSTR(ILTRDJ,2,5),'RRDDD')),'01-JAN-1900')
					INTO	DTA_ULT_MOV
					FROM	F4111_TONE
				...
				IF	CONTADOR = 0 THEN
					INFO_COMP := 'ULTIMA MOVIMENTAÇÃO (FILIAL '||FILIAL_ERP||') = '||DTA_MOV;
				ELSE
					TXT_COMP := ' - POSSUI LOCAL SECUNDÁRIO';
					INFO_COMP := 'ULTIMA MOVIMENTAÇÃO (FILIAL '||FILIAL_ERP||') = '||DTA_MOV||TXT_COMP;
				END IF;
			END IF;
			...
			EXCEPTION
				WHEN NO_DATA_FOUND THEN
					...
					INTO	UM_ERP,
							...
							UM_ALTER_ERP
					FROM	F4101_TONE
					WHERE	TRIM(IMLITM) = TO_CHAR(REC0.CODIGO_ANTIGO);
			END;
			...
			IF REC0.CONTROLE = 'N' THEN
				...
				--ATUALIZA CAMPO CONTROLE INDICANDO QUE O ITEM MESTRE FOI INSERIDO NO STAGING
				UPDATE	STAGING_ITEMMESTRE
				SET		CONTROLE = 'S'
				WHERE	TO_NUMBER(TRIM(COD_ANTIGO)) = REC0.CODIGO_ANTIGO
				AND		COD_ASTREIN = REC0.COD_ASTREIN;
			...
			ELSE
				...
				WHERE	cod_astrein = REC0.COD_ASTREIN
				AND		TO_NUMBER(TRIM(cod_antigo)) = REC0.CODIGO_ANTIGO
				...
				ELSE
					SELECT	NVL(TRIM(centro_erp),' '),
							...
							NVL(pt_reab_erp,0.00),
							NVL(prec_padr_erp,0.00),
							NVL(tam_fx_lt_erp,0.00),
							NVL(TRIM(TO_CHAR(info)), ' '),
							NVL(TRIM(TO_CHAR(class_mat)), ' '),
							NVL(TRIM(blq), ' '),
					...
					IF	(NVL(TRIM(FILIAL_ERP)		, ' ')	<>	NVL(TRIM(centro_erp_aux)		, ' ')	OR 
						 ...
						 NVL(TRIM(REC0.CLASS_MAT)	, ' ')	<>	NVL(TRIM(class_mat_aux)			, ' ')	OR 
						 NVL(TRIM(REC0.BLOQUEIO)	, ' ')	<>	NVL(TRIM(v_blq)					, ' '))	THEN
						UPDATE	STAGING_MATERIAL
						SET		centro_erp = FILIAL_ERP,
								...
								txt_lon_mm_erp = REC0.DESC_LONGA,
								ncm_erp = REC0.NCM,
								grp_merc = REC0.GRP_MERC,
								pos_dep_erp = POS_ERP,
								pt_reab_erp = PT_RB_ERP,
								pt_reab_san = PT_RB_ERP,
								tam_fx_lt_erp = TM_FX_LT_ERP,
								tam_fx_lt_san = TM_FX_LT_ERP,
								status = 'P',
								info = INFO_COMP,
								class_mat = REC0.CLASS_MAT,
								blq = REC0.BLOQUEIO,
								dta_ult_movi = DTA_MOV,
								qtd_bas = V_QTD_BAS,
					...
					ELSIF	(NVL(TRIM(TO_CHAR(INFO_COMP))		, ' ')	<> NVL(TRIM(TO_CHAR(info_aux))				, ' ')	OR 
							 NVL(TRIM(TO_CHAR(REC0.DESC_LONGA))	, ' ')	<> NVL(TRIM(TO_CHAR(txt_lon_mm_erp_aux))	, ' ')	OR 
							 NVL(TRIM(REC0.DESC_CURTA)			, ' ')	<> NVL(TRIM(desc_erp_aux)					, ' ')	OR 
							 NVL(TRIM(DTA_MOV)					, ' ')	<> NVL(TRIM(v_dta_ult_mov)					, ' ')	OR 
							 NVL(TRIM(REC0.GRP_MERC)			, ' ')	<> NVL(TRIM(grp_merc_aux)					, ' ')	OR 
							 NVL(TRIM(V_QTD_BAS)				, ' ')	<> NVL(TRIM(v_qtd_bas_aux)					, ' ')	OR 
							 NVL(TRIM(REC0.NCM)					, ' ')	<> NVL(TRIM(ncm_erp_aux)					, ' ')) THEN
						UPDATE	STAGING_MATERIAL
						SET		info = INFO_COMP,
								dta_ult_movi = DTA_MOV,
								qtd_bas = V_QTD_BAS,
								...
								grp_merc = REC0.GRP_MERC,
								txt_lon_mm_erp = REC0.DESC_LONGA,
								txt_lon_mm_san = REC0.DESC_LONGA,
								desc_erp = REC0.DESC_CURTA,
								desc_san = REC0.DESC_CURTA,
								ncm_erp = REC0.NCM
						WHERE	cod_astrein = REC0.COD_ASTREIN
						AND		TO_NUMBER(TRIM(cod_antigo)) = REC0.CODIGO_ANTIGO
						AND		centro_erp = FILIAL_ERP
	...
	UPDATE STAGING_CONTROLE SET DATA_PROC = SYSDATE WHERE DADO = 'MATERIAL';
	COMMIT;
